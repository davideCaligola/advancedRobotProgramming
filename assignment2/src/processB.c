...
int main(int argc, char const *argv[])
{
    ...
    if (argc != 2)
    {
        printf("expected 1 input arguments, received %d\n", argc - 1);
        exit(EXIT_FAILURE);
    }
    ...
    int sharedMem_fd = shm_open(SHARED_MEM_NAME, O_RDONLY, 0444);
    if (sharedMem_fd == -1) {
    ...
    pSemaphore_write = sem_open(SEMAPHORE_WRITE, 0);
    ...
    pSemaphore_read = sem_open(SEMAPHORE_READ, 0);
